...
def colorTrainStation(lineName, stationRef, value):
    ...
    circle = {
        ...
        "@cy": position.imag,
        "@r": "8",
        "@fill": mcolors.to_hex(cmap(value)),
    }
...
for lineName, lineStations in notServicedSectionDict.items():
    for stationIdx, station in enumerate(lineStations):
        ...
        colorTrainStation(lineName, lineStations[stationIdx][0], lineStations[stationIdx][2])
        

svgDict["svg"]["circle"] = listOfCircles
with open("stat_map.svg", "w") as outputSvg:
