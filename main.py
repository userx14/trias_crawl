...
if len(sys.argv)==1 or (sys.argv[1] == "live"):
    try:
    ...
    try:
        ...
        logging.error("Error during live graph rendering:\n%s", traceback.format_exc())

    #upload only new live data
    for src_path in www_dir.iterdir():
        if "live" in src_path.name:
...
if len(sys.argv)==1 or (sys.argv[1] == "stat"):
